...
When('I insert label {string}', async function (label) {
    let element = await this.driver.$('.ember-basic-dropdown.gh-token-input');
...
Then('I should be the page in the list with name {string}', async function (title) {
    const page = await this.driver.$('section.content-list').$('li.gh-list-row.gh-posts-list-item').$('h3')
...
Then('I delete all pages', async function () {
    const page = await this.driver.$('section.content-list').$$('li.gh-list-row.gh-posts-list-item')
    for (let i = 0; i < page.length; i++) {
        let pageItem = pageItems[i];
        await pageItem.click();
        await this.driver.pause(1000);
        let settingsButton = await this.driver.$('[title="Settings"]');
        await settingsButton.click();
        let deleteButton = await this.driver.$('.settings-menu-delete-button');
        await deleteButton.click();
        let confirmDeleteButton = await this.driver.$('button.gh-btn.gh-btn-red.gh-btn-icon.ember-view');
...
Then('I should be the member in the list with name {string}', async function (name) {
    const member = await this.driver.$('section.content-list').$('li.gh-list-row.gh-members-list-item').$('h3')
...
Then('I delete all members', async function () {
    const members = await this.driver.$('section.content-list').$$('li.gh-list-row.gh-members-list-item')
    for (let i = 0; i < members.length; i++) {
        let member = members[i];
        await member.click();
        await this.driver.pause(1000);
        let deleteButton = await this.driver.$('button.gh-btn.gh-btn-red');
        await deleteButton.click();
        let confirmDeleteButton = await this.driver.$('button.gh-btn.gh-btn-red.gh-btn-icon.ember-view');
        await confirmDeleteButton.click();
    }
...
When('ingreso el título {kraken-string}', async function (title) {
    let titleInput = await this.driver.$('textarea');
    await titleInput.setValue(title);
});

When('ingreso la descripción {kraken-string}', async function (description) {
    let descriptionInput = await this.driver.$('article');
...
When('guardo la página', async function () {
    let saveButton = await this.driver.$('.gh-btn-editor');
...
When('I click on the selection your profile', async function () {
    const profileLink = await this.driver.$('a[href="#/staff/jaime/"]');
...
Then('I should see user name as {kraken-string}', async function (expectedValue) {
    const inputField = await this.driver.$('#user-name');
